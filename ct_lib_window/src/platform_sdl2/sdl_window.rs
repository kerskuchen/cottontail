...
window_create(uint display_index, const char* title)
{
    ...
    result.refresh_rate = desktop_mode.refresh_rate;
    result.fullscreen_type = FULLSCREEN_TYPE_BORDERLESS_WINDOW;
...
#if CT_SLOW
    // NOTE: This is a debug version of 4.3
    ...
    {
        logerror("Failed to determine current display resolution: %s", SDL_GetError());
        return result;
    }
    int window_pos_x = SDL_WINDOWPOS_UNDEFINED;
    int window_pos_y = SDL_WINDOWPOS_UNDEFINED;
    int initial_window_width = display_mode.w;
    int initial_window_height = display_mode.h;

    uint32 flags = SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN;

#else
    int window_pos_x = SDL_WINDOWPOS_CENTERED_DISPLAY(display_index);
    int window_pos_y = SDL_WINDOWPOS_CENTERED_DISPLAY(display_index);
    int initial_window_width = 640;
    int initial_window_height = 360;
    uint32 flags = SDL_WINDOW_OPENGL;
#endif

    SDL_SetHint(SDL_HINT_VIDEO_MINIMIZE_ON_FOCUS_LOSS, "0");
    result.sdl_window = SDL_CreateWindow(title,
                                         window_pos_x,
                                         window_pos_y,
    ...
    logprint("OpenGL version loaded: %d.%d", GLVersion.major, GLVersion.minor);

    window_get_dimensions(&result, NULL, NULL);
