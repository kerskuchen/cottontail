...
pub struct Aseprite {
    ...
    pub animation_tags: HashMap<String, AnimationTag>,
    pub layers: Vec<ChunkLayer>,
    pub cels: HashMap<CelIndex, ChunkCel>,
}

impl Aseprite {
    pub fn get_cel(&self, frame_index: usize, layer_index: usize) -> Option<&ChunkCel> {
        assert!(frame_index < self.frames.len());
        assert!(layer_index < self.layers.len());

        let maybe_cel = self.cels.get(&CelIndex {
            frame_index,
            layer_index,
        });

        if maybe_cel.is_none() {
            return None;
        }

        let cel = maybe_cel.unwrap();
        match &cel.cel {
            aseprite_reader::Cel::Image { .. } => Some(cel),
            aseprite_reader::Cel::Linked {
                frame_index_to_link_with,
            } => {
                let real_cel = self
                    .cels
                    .get(&CelIndex {
                        frame_index: *frame_index_to_link_with,
                        layer_index,
                    })
                    .unwrap_or_else(|| {
                        panic!(
                            "Linked cel not found for (frame_index: {}, layer_index: {})",
                ...
                match &real_cel.cel {
                    aseprite_reader::Cel::Linked {
                        ...
                        unreachable!("Linked cell (frame_index: {}, layer_index: {}) points to another linked (frame_index: {}, layer_index: {})",
                        frame_index, layer_index, frame_index, frame_index_to_link_with);
                    }
                    _ => {}
                }
                Some(real_cel)
    ...
    #[must_use]
    pub fn render_cel_premultiplied(&self, frame_index: usize, layer_index: usize) -> Bitmap {
        ...
        if layer.flag_visible == false || layer.opacity == 0 {
            // Nothing is visible
            return result;
        }
    ...
    pub fn from_file(filepath: &str) -> Result<Aseprite, String> {
        ...
        for frame_index in 0..fileheader.frame_count {
            ...
            for chunk_index in 0..frameheader.chunk_count {
                ...
                match chunk_type {
                    ChunkType::Layer => {
                        let layer =
                            ChunkLayer::from_deserializer(&mut chunk_deserializer, layers.len())
                                .map_err(|error| {
                                    format!(
                    ...
                    ChunkType::Tags => {
                        let tags = ChunkTags::from_deserializer(&mut chunk_deserializer).map_err(
                            |error| {
                                format!(
...
impl ChunkCel {
    fn get_bitmap(&self) -> &Bitmap {
        match &self.cel {
            Cel::Linked { .. } => {
                panic!("Cannot get bitmap of linked cell")
            }
            Cel::Image { bitmap } => bitmap,
        }
    }
